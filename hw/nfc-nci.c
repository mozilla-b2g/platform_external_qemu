...
static void
perform_deactivate_transition(enum nci_rf_deactivation_type type,
                              struct nfc_device* nfc)
{
    unsigned long bits;
    enum nfc_rfst rfst;
...
nfc_delivery_deactivate_cmd_cb(void* data, union nci_packet* pkt)
{
    ...
    if (!send_ntf) {
        perform_deactivate_transition(param->type, nfc);
    } else {
                          ...
                          union nci_packet* ntf)
{
    ...
    perform_deactivate_transition(payload->type, nfc);
